...
如果地址通过质押获得了 Energy，或者收到了其他账户代理的 Energy，这部分资源可以直接用于合约执行。
...
如果已经有自有质押资源，可以考虑通过资源代理，把符合条件的 Energy 分配给真正需要发送 USDT 的地址。
...
偶发用户可以先检查现有资源，再决定是否临时获得 Energy；长期稳定的大量交易可以评估质押；多地址业务可以优化资源代理；自动化系统则应该把资源估算、补充和到账确认放到签名广播之前。
